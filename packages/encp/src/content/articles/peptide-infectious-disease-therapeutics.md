---
title: "Peptide Infectious Disease Therapeutics"
description: "Therapeutic peptides for infectious diseases including antivirals, antifungals, and antiparasitic agents."
status: "published"
author: "Encyclopeptide Editorial"
pubDate: 2024-01-15
tags: ['infectious-disease', 'antiviral', 'antifungal', 'therapeutic', 'antimicrobial']
category: "Therapeutic Peptides"
difficulty: "intermediate"
relatedArticles: ['antimicrobial-peptide-drugs', 'antifungal-peptide-drugs']
---

## Overview

Antimicrobial peptides represent a new class of therapeutics for drug-resistant infections, offering novel mechanisms of action.

## Approved Infectious Disease Peptides

| Drug            | Class       | Target        | Indication      |
| --------------- | ----------- | ------------- | --------------- |
| Enfuvirtide     | Fusion inhibitor | HIV gp41 | HIV infection   |
| Daptomycin      | Lipopeptide | Membrane      | Gram-positive   |
| Polymyxin B     | Lipopeptide | Membrane      | Gram-negative   |

## Mechanisms

- **Fusion inhibitors**: Block viral entry
- **Membrane disruptors**: Kill pathogens directly
- **Immune modulators**: Enhance host defense

## Chemical Identity

| Property | Value |
|----------|-------|
| **Name** | Peptide Infectious Disease Therapeutics |
| **Category** | Therapeutic Peptides |

## References

1. Encyclopeptide Database. "Peptide Infectious Disease Therapeutics" monograph. encyclopeptide.com.
